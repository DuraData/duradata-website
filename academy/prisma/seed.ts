...
async function main() {
  await prisma.platformSetting.upsert({
    ...
    update: { value: "Duradata Academy" },
    create: { key: "platformName", value: "Duradata Academy" },
  })
  await prisma.platformSetting.upsert({
    where: { key: "supportEmail" },
    update: { value: "academy@duradata.co.za" },
    create: { key: "supportEmail", value: "academy@duradata.co.za" },
  })
  await prisma.platformSetting.upsert({
  ...
  await prisma.user.upsert({
    ...
    create: { email: "admin@academy.local", name: "Naledi Admin", role: "admin", status: "active" },
  })

  const instructor = await prisma.user.upsert({
    where: { email: "instructor@academy.local" },
    update: { name: "Sarah Instructor", role: "instructor", status: "active" },
    create: { email: "instructor@academy.local", name: "Sarah Instructor", role: "instructor", status: "active" },
  })

  const student = await prisma.user.upsert({
    where: { email: "student@academy.local" },
    update: { name: "John Student", role: "student", status: "active" },
    create: { email: "student@academy.local", name: "John Student", role: "student", status: "active" },
  })

  const applicant = await prisma.user.upsert({
    where: { email: "applicant@academy.local" },
    update: { name: "Thabo Applicant", role: "student", status: "active" },
    create: { email: "applicant@academy.local", name: "Thabo Applicant", role: "student", status: "active" },
  })
    ...
    data: [
      {
        ...
        body: "Holiday sessions will be posted in the Academic Learning Hub. Keep an eye on Live Lessons for upcoming dates.",
        grade: 12,
        subject: "Mathematics",
    ...
    data: [
      {
        ...
        userId: student.id,
        title: "Welcome to Duradata Academy",
