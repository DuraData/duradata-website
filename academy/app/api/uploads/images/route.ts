...
async function featureAvailable(entityType: "course" | "tutorial") {
  const features = await getAcademyFeatures()
  return entityType === "course" ? features.corporateLearningEnabled : features.freeLearningEnabled
}
...
export async function POST(req: Request) {
  ...
  if (!(await featureAvailable(parsed.data.entityType))) return Response.json({ error: "Learning mode is currently disabled" }, { status: 404 })
  try {
