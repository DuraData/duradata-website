...
const SettingSchema = z.object({
  key: z.enum([
    ...
    "commissionRateBps",
    "payoutMinimumZar",
    "academicLearningEnabled",
    "corporateLearningEnabled",
    "freeLearningEnabled",
  ]),
  value: z.string().max(10_000),
}).superRefine((setting, context) => {
  if (setting.key.endsWith("LearningEnabled") && !["true", "false"].includes(setting.value)) {
    context.addIssue({ code: "custom", message: "Learning mode values must be true or false" })
  }
})
