...
// check that the MySQL connection pool (a default `connection_limit=5` per
// application process) holds up under concurrency without
// surfacing pool-exhaustion errors to callers.
//
// This hits a *running* server (local `next dev`/`next start`, or a deployed
// deployed Hostinger URL) over HTTP — it does not open its own direct MySQL
// connections, so it measures the same code path real traffic would hit.
//
// Usage:
//   npx tsx scripts/db-resilience-check.ts
//   BASE_URL=https://academy.duradata.co.za CONCURRENCY=30 TOTAL_REQUESTS=300 npx tsx scripts/db-resilience-check.ts
...
async function main() {
  ...
  if (poolErrors.length > 0) {
    console.error(
      `\n❌ FAIL: ${poolErrors.length} request(s) show MySQL/Prisma pool-exhaustion signatures. ` +
        `Consider lowering CONCURRENCY, raising the database URL's \`connection_limit\`, or checking the MySQL provider's connection limit for this compute size.`
    )
    process.exit(1)
  }
