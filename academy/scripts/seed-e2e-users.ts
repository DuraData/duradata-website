...
//   depend on those, so it gets its own e2e.*@duradata-academy.test accounts instead.
//
// Safe to re-run (upserts). Run against whichever ACADEMY_DATABASE_URL is active —
// prefer a disposable local or isolated CI MySQL database over production.
...
const accounts = [
  {
    email: process.env.E2E_STUDENT_EMAIL?.trim() || "e2e.student@duradata-academy.test",
    password: process.env.E2E_STUDENT_PASSWORD?.trim() || PASSWORD_FALLBACK,
    name: "E2E Student",
    role: "student" as const,
  },
  {
    email: process.env.E2E_INSTRUCTOR_EMAIL?.trim() || "e2e.instructor@duradata-academy.test",
    password: process.env.E2E_INSTRUCTOR_PASSWORD?.trim() || PASSWORD_FALLBACK,
    name: "E2E Instructor",
    role: "instructor" as const,
  },
  {
    email: process.env.E2E_CONTENT_MANAGER_EMAIL?.trim() || "e2e.contentmanager@duradata-academy.test",
  ...
  {
    email: process.env.E2E_ADMIN_EMAIL?.trim() || "e2e.admin@duradata-academy.test",
