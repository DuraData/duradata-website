...
test.describe("Free and mocked paid fulfillment", () => {
  ...
  test.afterEach(async ({ page }) => {
    ...
    await api(page, "/api/admin/settings", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ settings: [{ key: "academicLearningEnabled", value: "false" }] }) })
  })
  ...
  test("free and paid subject packages enforce entitlement rules", async ({ page }) => {
    await loginAs(page, "admin")
    expect((await api(page, "/api/admin/settings", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ settings: [{ key: "academicLearningEnabled", value: "true" }] }) })).status).toBe(200)
    await loginAs(page, "student")
    ...
    const freeCheckout = await api<{ enrolledFree?: boolean; alreadyEnrolled?: boolean }>(page, "/api/checkout/prepare", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ itemType: "training", itemId: free.id }) })
    expect(freeCheckout.json.enrolledFree || freeCheckout.json.alreadyEnrolled).toBeTruthy()
    ...
    const checkout = await api<{ checkout?: { reference: string }; alreadyEnrolled?: boolean }>(page, "/api/checkout/prepare", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ itemType: "training", itemId: paid.id }) })
    if (checkout.json.checkout) {
      ...
      expect(checkout.json.alreadyEnrolled).toBe(true)
    }
