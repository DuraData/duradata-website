...
export async function searchPublishedContent(rawQuery: string): Promise<SearchResult[]> {
  ...
  const [courses, subjects, tutorials, lessons] = await Promise.all([
    ...
    features.freeLearningEnabled ? prisma.tutorial.findMany({ where: { status: "published", OR: [{ title: contains }, { shortDescription: contains }, { description: contains }] }, select: { id: true, slug: true, title: true, shortDescription: true }, take: 8 }) : [],
    features.freeLearningEnabled ? prisma.tutorialLesson.findMany({ where: { isPublished: true, section: { tutorial: { status: "published" } }, OR: [{ title: contains }, { summary: contains }] }, select: { id: true, slug: true, title: true, summary: true, section: { select: { tutorial: { select: { slug: true, title: true } } } } }, take: 12 }) : [],
  ])
  return [
    ...courses.map((item) => ({ id: item.id, type: "Course" as const, title: item.title, description: item.description, href: `/course/${item.id}` })),
    ...subjects.map((item) => ({ id: item.id, type: "Subject" as const, title: item.title, description: item.description, href: `/academic-learning/${item.id}` })),
