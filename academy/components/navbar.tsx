...
type Features = { academicLearningEnabled: boolean; corporateLearningEnabled: boolean; freeLearningEnabled: boolean }
const closedFeatures: Features = { academicLearningEnabled: false, corporateLearningEnabled: false, freeLearningEnabled: false }
...
export function Navbar({ initialFeatures = closedFeatures }: { initialFeatures?: Features }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [sessionRes, featureRes] = await Promise.all([
        ...
        fetch("/api/academy/features", { cache: "no-store" }).catch(() => null),
      ])
      const json = sessionRes ? await sessionRes.json().catch(() => null) : null
      const featureJson = featureRes ? await featureRes.json().catch(() => null) : null
      if (!cancelled) {
        setSession(json?.session ?? null)
        if (featureJson) setFeatures(featureJson)
      }
    }
    load()
    return () => {
        ...
        <div className="flex h-16 items-center justify-between">
          <div className="flex items-center gap-8">
            <Link href="/" className="flex items-center gap-2">
              ...
              <span className="text-lg font-semibold tracking-tight text-foreground">Duradata Academy</span>
            </Link>
            <div className="hidden md:flex items-center gap-6">
              {features.corporateLearningEnabled ? <Link href="/courses" className={navLinkClass}>
                Courses
              </Link> : null}
              {features.corporateLearningEnabled ? <Link href="/categories" className={navLinkClass}>
                Categories
              </Link> : null}
              {features.freeLearningEnabled ? <Link href="/learn" className={navLinkClass}>
                Learn Free
              </Link> : null}
              {features.corporateLearningEnabled ? <Link href="/for-business" className={navLinkClass}>
                For Business
              </Link> : null}
              {features.academicLearningEnabled ? <Link href="/academic-learning" className={navLinkClass}>
                Academic Learning
              </Link> : null}
            </div>
          </div>
      ...
      {mobileMenuOpen && (
        <div className="border-t border-border bg-background md:hidden">
          <div className="space-y-2 px-4 py-4">
            {features.corporateLearningEnabled ? <Link href="/courses" className="block rounded-md px-2 py-2 text-sm font-medium text-foreground/75 transition-colors hover:bg-muted hover:text-foreground">
              Courses
            </Link> : null}
            {features.corporateLearningEnabled ? <Link href="/categories" className="block rounded-md px-2 py-2 text-sm font-medium text-foreground/75 transition-colors hover:bg-muted hover:text-foreground">
              Categories
            </Link> : null}
            {features.freeLearningEnabled ? <Link href="/learn" className="block rounded-md px-2 py-2 text-sm font-medium text-foreground/75 transition-colors hover:bg-muted hover:text-foreground">
              Learn Free
            </Link> : null}
            {features.corporateLearningEnabled ? <Link href="/for-business" className="block rounded-md px-2 py-2 text-sm font-medium text-foreground/75 transition-colors hover:bg-muted hover:text-foreground">
              For Business
            </Link> : null}
            {features.academicLearningEnabled ? <Link href="/academic-learning" className="block rounded-md px-2 py-2 text-sm font-medium text-foreground/75 transition-colors hover:bg-muted hover:text-foreground">
              Academic Learning
            </Link> : null}
